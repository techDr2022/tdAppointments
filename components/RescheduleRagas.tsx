...
import { SendRescheduleMessageRagas } from "@/actions/SendMessageBmt";
...
const RescheduleRagasModal: React.FC<RescheduleModalProps> = ({
  isOpen,
  onClose,
  appointmentId,
}) => {
  ...
  const handleReschedule = async () => {
    if (selectedDate && selectedTime) {
      ...
      if (appointmentId) {
        setIsprocess(true);
        const result = await SendRescheduleMessageRagas({
          appointmentId,
          selectedDate,
          selectedTime,
        });
        ...
        <div className="inline-block w-full max-w-md my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-2xl rounded-2xl relative z-50">
          {success ? (
            ...
            <div className="p-6">
              ...
              {/* Time Slots Section */}
              {selectedDate && timeSlotsMemo.length > 0 && (
                <div className="bg-gray-50 rounded-lg p-4 mt-4">
                  <h3 className="text-sm font-semibold text-black mb-2">
                    Available Time Slots
                  </h3>
                  <div className="grid grid-cols-3 gap-2">
                    {timeSlotsMemo.map(({ time, ampm }) => (
                      <button
                        key={time}
...
export default RescheduleRagasModal;
